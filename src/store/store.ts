...
export const useStore = create(
  persist((set, get) => ({
    ...
    addToFavoritesList: (type: string, id: string) => set(produce(state => {
      if (type == 'Coffee') {
    ...
    decrementCartItemQuantity: (id: string, size: string) => set(produce(state => {
      for (let i = 0; i < state.CartList.length; i++) {
        if (state.CartList[i].id == id) {
          for (let j = 0; j < state.CartList[i].prices.length; j++) {
            if (state.CartList[i].prices[j].size = size) {
              if (state.CartList[i].prices.length > 1) { // size > 1
    ...
    addToOrderHistoryListFromCart: () => set(produce(state => {
      let temp = state.CartList.reduce(
        (accumulator: number, currentValue: any) => accumulator + parseFloat(currentValue.ItemPrice), 0,
      );
